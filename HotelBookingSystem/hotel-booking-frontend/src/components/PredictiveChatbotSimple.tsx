...
import { ChatbotMessage, ChatbotSuggestion, PredictiveChatbotData } from '../types/chatbot';
import { useAuth } from '../contexts/AuthContext';

interface PredictiveChatbotSimpleProps {
  onClose?: () => void;
  expanded?: boolean;
}

export const PredictiveChatbot: React.FC<PredictiveChatbotSimpleProps> = ({ 
  onClose, 
  expanded = false 
}) => {
  const { user } = useAuth();
    ...
    const initialSuggestions: ChatbotSuggestion[] = [
      {
        ...
        type: 'quick_question',
        parameters: { query: 'What is the room availability for next weekend?' }
      },
      {
        text: 'Analyze pricing trends',
        type: 'quick_question',
        parameters: { query: 'Show me pricing trends for the next 30 days' }
      },
      {
        text: 'Demand forecast',
        type: 'quick_question',
        parameters: { query: 'What is the demand forecast for luxury rooms?' }
      }
    ];
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await EnhancedAnalyticsService.queryChatbot({
            ...
            <Paper key={index} sx={{ p: 2 }}>
              <Box display="flex" justifyContent="space-between" alignItems="center">
                <Typography variant="body2" color="text.secondary">
                  {EnhancedAnalyticsService.formatDate(prediction.predictionDate)}
                </Typography>
                <Typography variant="h6" color="primary">
                  {EnhancedAnalyticsService.formatCurrency(prediction.predictedPrice)}
                </Typography>
                {prediction.confidenceLevel && (
                  <Typography variant="caption" color="text.secondary">
                    {EnhancedAnalyticsService.formatPercentage(prediction.confidenceLevel)} confidence
                  </Typography>
                )}
              </Box>
            </Paper>
        ...
        <Stack spacing={2}>
          {data.availabilityForecasts.slice(0, 4).map((forecast, index) => (
            <Paper key={index} sx={{ p: 2 }}>
              <Box display="flex" justifyContent="space-between" alignItems="center">
                <Typography variant="body2" color="text.secondary">
                  {EnhancedAnalyticsService.formatDate(forecast.forecastDate)}
                </Typography>
